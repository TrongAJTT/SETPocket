import 'dart:convert';
import 'dart:io';
import 'package:http/http.dart' as http;
import 'package:setpocket/services/app_logger.dart';
import 'package:setpocket/services/settings_service.dart';

enum CurrencyStatus {
  success,
  failed,
  timeout,
  notSupported,
  staticRate,
  fetchedRecently,
  fetching // New status for currencies being fetched
}

// New enum for currency value status (separate from fetch status)
enum CurrencyValueStatus {
  recentlyUpdated, // successful and lastModified within 1 hour
  updated, // successful and has lastModified data
  static // no fetch data or using static rates
}

class CurrencyFetchResult {
  final double rate;
  final CurrencyStatus status;
  final DateTime? fetchDate; // New field to track when currency was fetched

  CurrencyFetchResult(
      {required this.rate, required this.status, this.fetchDate});
}

// Custom HTTP-based currency fetcher to replace live_currency_rate
class CustomCurrencyFetcher {
  static const String _baseUrl = 'https://api.exchangerate-api.com/v4/latest';
  static const Duration _timeout = Duration(seconds: 10);

  static Future<double> fetchRate(
      String fromCurrency, String toCurrency) async {
    try {
      // Create HTTP client with proper headers
      final client = http.Client();

      // Set up headers for better compatibility
      final headers = {
        'User-Agent': 'SETPocket/1.0.2 (Flutter App)',
        'Accept': 'application/json',
        'Accept-Encoding': 'gzip, deflate',
        'Connection': 'keep-alive',
      };

      final uri = Uri.parse('$_baseUrl/$fromCurrency');
      logInfo('CustomCurrencyFetcher: Fetching from $uri');

      final response =
          await client.get(uri, headers: headers).timeout(_timeout);

      client.close();

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final rates = data['rates'] as Map<String, dynamic>;

        if (rates.containsKey(toCurrency)) {
          final rate = (rates[toCurrency] as num).toDouble();
          logInfo('CustomCurrencyFetcher: Got rate for $toCurrency: $rate');
          return rate;
        } else {
          throw Exception('Currency $toCurrency not found in response');
        }
      } else {
        throw HttpException(
            'HTTP ${response.statusCode}: ${response.reasonPhrase}');
      }
    } on SocketException catch (e) {
      logError('CustomCurrencyFetcher: Network error: $e');
      throw Exception('Network error: Check internet connection');
    } on HttpException catch (e) {
      logError('CustomCurrencyFetcher: HTTP error: $e');
      throw Exception('HTTP error: $e');
    } on FormatException catch (e) {
      logError('CustomCurrencyFetcher: JSON parse error: $e');
      throw Exception('Invalid response format');
    } catch (e) {
      logError('CustomCurrencyFetcher: Unexpected error: $e');
      throw Exception('Failed to fetch currency rate: $e');
    }
  }
}

class CurrencyService {
  // Extended static rates for fallback (Updated December 2024)
  static const Map<String, double> _staticRates = {
    'USD': 1.0, // Base currency
    'EUR': 0.8769,
    'GBP': 0.7387,
    'JPY': 144.67,
    'CAD': 1.3694,
    'AUD': 1.5393,
    'VND': 26052.17,
    'CNY': 7.1893,
    'INR': 85.84,
    'KRW': 1422.50,
    'SGD': 1.2873,
    'THB': 32.69,
    'MYR': 4.2281,
    'IDR': 16307.83,
    'PHP': 55.84,
    'CHF': 0.8217,
    'SEK': 10.58,
    'NOK': 10.10,
    'DKK': 6.53,
    'PLN': 3.96,
    'CZK': 21.71,
    'HUF': 353.57,
    'RON': 4.42,
    'BGN': 1.715,
    'HRK': 6.58,
    'RUB': 78.93,
    'TRY': 39.25,
    'BRL': 5.57,
    'MXN': 20.15,
    'ARS': 1015.50,
    'CLP': 965.00,
    'COP': 4112.07,
    'PEN': 3.63,
    'UYU': 43.20,
    'ZAR': 17.85,
    'EGP': 49.64,
    'MAD': 9.72,
    'NGN': 1561.69,
    'KES': 129.50,
    'GHS': 15.85,
    'UGX': 3638.47,
    'TZS': 2660.66,
    'ETB': 133.75,
    'XOF': 575.00,
    'XAF': 575.00,
    'ILS': 3.52,
    'SAR': 3.75,
    'AED': 3.67,
    'QAR': 3.64,
    'KWD': 0.304,
    'BHD': 0.376,
    'OMR': 0.3845,
    'JOD': 0.709,
    'LBP': 89500.0,
    'IRR': 42350.0,
    'PKR': 282.71,
    'BDT': 119.85,
    'LKR': 291.50,
    'NPR': 134.28,
    'BTN': 85.84,
    'MVR': 15.42,
    'AFN': 70.25,
    'UZS': 12850.0,
    'KZT': 523.50,
    'KGS': 86.75,
    'TJS': 10.85,
    'TMT': 3.50,
    'GEL': 2.73,
    'AMD': 383.95,
    'AZN': 1.70,
    'BYN': 3.28,
    'MDL': 17.85,
    'UAH': 41.50,
    'HKD': 7.85,
    'TWD': 29.84,
    'MOP': 8.08,
    'BND': 1.2873,
    'LAK': 21630.65,
    'KHR': 4050.0,
    'MMK': 2101.84,
    'FJD': 2.25,
    'PGK': 4.02,
    'SBD': 8.57,
    'TOP': 2.35,
    'VUV': 118.50,
    'WST': 2.75,
    'XPF': 104.25,
    'NZD': 1.65,
  };

  // Get static rates for fallback
  static Map<String, double> getStaticRates() {
    return Map.from(_staticRates);
  }

  static DateTime? _lastUpdated;
  static bool _isUsingLiveRates = false;
  static Map<String, double> _currentRates = _staticRates;
  static final Map<String, CurrencyStatus> _currencyStatus = {};

  // Get available currencies - extensive list
  static List<Currency> getSupportedCurrencies() {
    return [
      // Major currencies
      const Currency('USD', 'US Dollar', '\$'),
      const Currency('EUR', 'Euro', '€'),
      const Currency('GBP', 'British Pound', '£'),
      const Currency('JPY', 'Japanese Yen', '¥'),
      const Currency('CAD', 'Canadian Dollar', 'C\$'),
      const Currency('AUD', 'Australian Dollar', 'A\$'),
      const Currency('CHF', 'Swiss Franc', 'CHF'),

      // Asian currencies
      const Currency('VND', 'Vietnamese Dong', '₫'),
      const Currency('CNY', 'Chinese Yuan', '¥'),
      const Currency('HKD', 'Hong Kong Dollar', 'HK\$'),
      const Currency('TWD', 'Taiwan Dollar', 'NT\$'),
      const Currency('SGD', 'Singapore Dollar', 'S\$'),
      const Currency('MYR', 'Malaysian Ringgit', 'RM'),
      const Currency('THB', 'Thai Baht', '฿'),
      const Currency('IDR', 'Indonesian Rupiah', 'Rp'),
      const Currency('PHP', 'Philippine Peso', '₱'),
      const Currency('INR', 'Indian Rupee', '₹'),
      const Currency('KRW', 'South Korean Won', '₩'),
      const Currency('BND', 'Brunei Dollar', 'B\$'),
      const Currency('LAK', 'Lao Kip', '₭'),
      const Currency('KHR', 'Cambodian Riel', '៛'),
      const Currency('MMK', 'Myanmar Kyat', 'K'),
      const Currency('MOP', 'Macau Pataca', 'MOP'),

      // European currencies
      const Currency('SEK', 'Swedish Krona', 'kr'),
      const Currency('NOK', 'Norwegian Krone', 'kr'),
      const Currency('DKK', 'Danish Krone', 'kr'),
      const Currency('PLN', 'Polish Zloty', 'zł'),
      const Currency('CZK', 'Czech Koruna', 'Kč'),
      const Currency('HUF', 'Hungarian Forint', 'Ft'),
      const Currency('RON', 'Romanian Leu', 'lei'),
      const Currency('BGN', 'Bulgarian Lev', 'лв'),
      const Currency('HRK', 'Croatian Kuna', 'kn'),
      const Currency('RUB', 'Russian Ruble', '₽'),
      const Currency('TRY', 'Turkish Lira', '₺'),
      const Currency('UAH', 'Ukrainian Hryvnia', '₴'),
      const Currency('BYN', 'Belarusian Ruble', 'Br'),
      const Currency('MDL', 'Moldovan Leu', 'L'),
      const Currency('GEL', 'Georgian Lari', '₾'),
      const Currency('AMD', 'Armenian Dram', '֏'),
      const Currency('AZN', 'Azerbaijani Manat', '₼'),

      // Middle East & Africa
      const Currency('ILS', 'Israeli Shekel', '₪'),
      const Currency('SAR', 'Saudi Riyal', '﷼'),
      const Currency('AED', 'UAE Dirham', 'د.إ'),
      const Currency('QAR', 'Qatari Riyal', '﷼'),
      const Currency('KWD', 'Kuwaiti Dinar', 'د.ك'),
      const Currency('BHD', 'Bahraini Dinar', '.د.ب'),
      const Currency('OMR', 'Omani Rial', '﷼'),
      const Currency('JOD', 'Jordanian Dinar', 'د.ا'),
      const Currency('LBP', 'Lebanese Pound', '£'),
      const Currency('EGP', 'Egyptian Pound', 'E£'),
      const Currency('MAD', 'Moroccan Dirham', 'د.م.'),
      const Currency('ZAR', 'South African Rand', 'R'),
      const Currency('NGN', 'Nigerian Naira', '₦'),
      const Currency('KES', 'Kenyan Shilling', 'KSh'),
      const Currency('GHS', 'Ghanaian Cedi', '₵'),
      const Currency('UGX', 'Ugandan Shilling', 'USh'),
      const Currency('TZS', 'Tanzanian Shilling', 'TSh'),
      const Currency('ETB', 'Ethiopian Birr', 'Br'),
      const Currency('XOF', 'West African CFA Franc', 'CFA'),
      const Currency('XAF', 'Central African CFA Franc', 'FCFA'),

      // Americas
      const Currency('BRL', 'Brazilian Real', 'R\$'),
      const Currency('MXN', 'Mexican Peso', '\$'),
      const Currency('ARS', 'Argentine Peso', '\$'),
      const Currency('CLP', 'Chilean Peso', '\$'),
      const Currency('COP', 'Colombian Peso', '\$'),
      const Currency('PEN', 'Peruvian Sol', 'S/'),
      const Currency('UYU', 'Uruguayan Peso', '\$U'),

      // Central Asia
      const Currency('KZT', 'Kazakhstani Tenge', '₸'),
      const Currency('UZS', 'Uzbekistani Som', 'soʻm'),
      const Currency('KGS', 'Kyrgyzstani Som', 'лв'),
      const Currency('TJS', 'Tajikistani Somoni', 'ЅМ'),
      const Currency('TMT', 'Turkmenistani Manat', 'T'),
      const Currency('AFN', 'Afghan Afghani', '؋'),

      // South Asia
      const Currency('PKR', 'Pakistani Rupee', '₨'),
      const Currency('BDT', 'Bangladeshi Taka', '৳'),
      const Currency('LKR', 'Sri Lankan Rupee', '₨'),
      const Currency('NPR', 'Nepalese Rupee', '₨'),
      const Currency('BTN', 'Bhutanese Ngultrum', 'Nu.'),
      const Currency('MVR', 'Maldivian Rufiyaa', '.ރ'),

      // Pacific
      const Currency('NZD', 'New Zealand Dollar', 'NZ\$'),
      const Currency('FJD', 'Fijian Dollar', 'FJ\$'),
      const Currency('PGK', 'Papua New Guinea Kina', 'K'),
      const Currency('SBD', 'Solomon Islands Dollar', 'SI\$'),
      const Currency('TOP', 'Tongan Paʻanga', 'T\$'),
      const Currency('VUV', 'Vanuatu Vatu', 'VT'),
      const Currency('WST', 'Samoan Tala', 'WS\$'),
      const Currency('XPF', 'CFP Franc', '₣'),
    ];
  }

  // Convert currency - now uses cache service
  static double convert(double amount, String fromCurrency, String toCurrency) {
    if (fromCurrency == toCurrency) return amount;

    // Try to get rates from cache, fallback to current rates if cache not available
    Map<String, double>? cachedRates;
    try {
      // Note: We can't use await here, so we'll use the cached rates if available
      // The rates should be updated through the cache service when needed
      cachedRates = _currentRates.isNotEmpty ? _currentRates : _staticRates;
    } catch (e) {
      cachedRates = _staticRates;
    }

    final fromRate =
        cachedRates[fromCurrency] ?? _staticRates[fromCurrency] ?? 1.0;
    final toRate = cachedRates[toCurrency] ?? _staticRates[toCurrency] ?? 1.0;

    // Convert to USD first, then to target currency
    final usdAmount = amount / fromRate;
    return usdAmount * toRate;
  }

  // Get exchange rate between two currencies
  static double getExchangeRate(String fromCurrency, String toCurrency) {
    return convert(1.0, fromCurrency, toCurrency);
  }

  // Get last updated time
  static String getLastUpdated() {
    if (_lastUpdated != null) {
      return _lastUpdated!
          .toString()
          .substring(0, 19); // Format: YYYY-MM-DD HH:MM:SS
    }
    return 'Static rates (no live data)';
  }

  // Check if using live rates
  static bool get isUsingLiveRates => _isUsingLiveRates;

  // Initialize or refresh rates
  static Future<void> refreshRates() async {
    try {
      final newRates = await fetchLiveRates();
      _currentRates = newRates;
    } catch (e) {
      logError('Failed to refresh rates: $e');
      _currentRates = _staticRates;
      _isUsingLiveRates = false;
    }
  } // Progress tracking

  static void Function(String currency, CurrencyStatus status)? _onProgress;
  static bool _isCancelled = false;

  // Set progress callback
  static void setProgressCallback(
      void Function(String currency, CurrencyStatus status)? callback) {
    _onProgress = callback;
  }

  // Cancel current fetch operation
  static void cancelFetch() {
    _isCancelled = true;
  }

  // Future method for real API integration

  // Map to track last fetch time for each currency
  static final Map<String, DateTime> _currencyFetchTimes = {};

  // Get last fetch time for a currency
  static DateTime? getCurrencyLastFetchTime(String currencyCode) {
    return _currencyFetchTimes[currencyCode];
  }

  // Update currency fetch time (used by cache service)
  static void updateCurrencyFetchTime(String currencyCode, DateTime fetchTime) {
    _currencyFetchTimes[currencyCode] = fetchTime;
  }

  // Load status from cache (used by cache service)
  static void updateCurrencyStatuses(Map<String, CurrencyStatus> statuses) {
    _currencyStatus.clear();
    _currencyStatus.addAll(statuses);
  }

  // Check if currency needs refresh (older than 1 hour)
  static bool currencyNeedsRefresh(String currencyCode) {
    final fetchTime = _currencyFetchTimes[currencyCode];
    if (fetchTime == null) return true;

    final now = DateTime.now();
    final difference = now.difference(fetchTime);
    return difference.inHours >= 1;
  }

  static Future<Map<String, double>> fetchLiveRates() async {
    try {
      logInfo('CurrencyService: Starting to fetch live rates...');
      _isCancelled = false; // Reset cancel flag
      var rates = <String, double>{};
      bool hasLiveData = false;

      // Update status transitions before fetch
      updateStatusBeforeFetch();

      // Add USD as base
      rates['USD'] = 1.0;
      _currencyStatus['USD'] = CurrencyStatus.success;
      _currencyFetchTimes['USD'] = DateTime.now();
      // Only call progress callback if not cancelled
      if (!_isCancelled) {
        _onProgress?.call('USD', CurrencyStatus.success);
      }
      logInfo('CurrencyService: Added USD base rate');

      // Try to get rates for each currency - PARALLEL FETCHING for better performance
      final currencies = getSupportedCurrencies();
      logInfo(
          'CurrencyService: Fetching rates for ${currencies.length} currencies in parallel...');

      // Create futures for all currency fetches (except USD)
      final fetchFutures = <String, Future<CurrencyFetchResult>>{};
      for (final currency in currencies) {
        if (currency.code != 'USD') {
          // Only fetch if currency needs refresh (older than 1 hour) or had failed/timeout status
          final needsRefresh = currencyNeedsRefresh(currency.code);
          final previousStatus = _currencyStatus[currency.code];
          final shouldFetch = needsRefresh ||
              previousStatus == CurrencyStatus.failed ||
              previousStatus == CurrencyStatus.timeout ||
              previousStatus == CurrencyStatus.staticRate ||
              previousStatus == null;

          if (shouldFetch) {
            // Mark as fetching
            markCurrencyAsFetching(currency.code);
            // Only call progress callback if not cancelled
            if (!_isCancelled) {
              _onProgress?.call(currency.code, CurrencyStatus.fetching);
            }

            fetchFutures[currency.code] =
                _fetchSingleRateWithStatus(currency.code);
            logInfo(
                'CurrencyService: Will fetch ${currency.code} (needs refresh: $needsRefresh, previous status: $previousStatus)');
          } else {
            // Skip fetch but report current status for progress dialog
            // Only call progress callback if not cancelled
            if (!_isCancelled) {
              _onProgress?.call(currency.code, previousStatus);
            }
            logInfo(
                'CurrencyService: Skipping fetch for ${currency.code} (recently fetched)');
          }
        }
      }

      // Single fetch attempt - no retries here anymore
      if (fetchFutures.isNotEmpty) {
        final results = await Future.wait(
          fetchFutures.entries.map((entry) async {
            if (_isCancelled) {
              // Don't call progress callback for cancelled items
              return MapEntry(
                  entry.key,
                  CurrencyFetchResult(
                      rate: _staticRates[entry.key] ?? 1.0,
                      status: CurrencyStatus.failed));
            }
            try {
              // Get timeout from settings service
              final timeoutSeconds = await SettingsService.getFetchTimeout();
              final result =
                  await entry.value.timeout(Duration(seconds: timeoutSeconds));

              // Update fetch time for this currency if successful
              if (result.status == CurrencyStatus.success &&
                  result.fetchDate != null) {
                _currencyFetchTimes[entry.key] = result.fetchDate!;
              }

              // Only call progress callback if not cancelled
              if (!_isCancelled) {
                _onProgress?.call(entry.key, result.status);
              }
              return MapEntry(entry.key, result);
            } catch (e) {
              logError('CurrencyService: Timeout/Error for ${entry.key}: $e');
              // Only call progress callback if not cancelled
              if (!_isCancelled) {
                _onProgress?.call(entry.key, CurrencyStatus.timeout);
              }
              return MapEntry(
                  entry.key,
                  CurrencyFetchResult(
                      rate: _staticRates[entry.key] ?? 1.0,
                      status: CurrencyStatus.timeout));
            }
          }),
        );

        // Process results
        for (final result in results) {
          rates[result.key] = result.value.rate;
          _currencyStatus[result.key] = result.value.status;
          if (result.value.status == CurrencyStatus.success) {
            hasLiveData = true;
          }
        }
      }

      // Add currencies that weren't fetched (using existing values)
      for (final currency in currencies) {
        if (currency.code != 'USD' && !rates.containsKey(currency.code)) {
          // Get rate from current rates
          final rate = _currentRates[currency.code] ??
              _staticRates[currency.code] ??
              1.0;
          rates[currency.code] = rate;

          // Keep existing status if not fetched
          if (!_currencyStatus.containsKey(currency.code)) {
            _currencyStatus[currency.code] = CurrencyStatus.staticRate;
          }
        }
      }

      // Update tracking variables
      if (hasLiveData) {
        _lastUpdated = DateTime.now();
        _isUsingLiveRates = true;
        logInfo('CurrencyService: Successfully fetched live currency rates');
      } else {
        _isUsingLiveRates = false;
        logInfo(
            'CurrencyService: Using static currency rates (live data unavailable)');
      }

      logInfo('CurrencyService: Final rates count: ${rates.length}');
      logInfo(
          'CurrencyService: Sample rates: ${rates.entries.take(3).map((e) => '${e.key}: ${e.value}').join(', ')}');

      // Update current rates for use in convert() method
      _currentRates = Map<String, double>.from(rates);
      logInfo('CurrencyService: Updated _currentRates with new data');

      return rates;
    } catch (e) {
      logError('CurrencyService: Error fetching live rates: $e');
      // Fall back to static rates
      _isUsingLiveRates = false;
      await Future.delayed(const Duration(milliseconds: 500));
      logInfo('CurrencyService: Returning static rates as fallback');
      return Map<String, double>.from(_staticRates);
    }
  }

  // Enhanced method to fetch rate with status tracking
  static Future<CurrencyFetchResult> _fetchSingleRateWithStatus(
      String currencyCode) async {
    try {
      // Check if cancelled before starting actual fetch
      if (_isCancelled) {
        logInfo(
            'CurrencyService: Fetch cancelled for $currencyCode before API call');
        return CurrencyFetchResult(
            rate: _staticRates[currencyCode] ?? 1.0,
            status: CurrencyStatus.failed);
      }

      logInfo('CurrencyService: Fetching rate for $currencyCode...');
      final result = await CustomCurrencyFetcher.fetchRate('USD', currencyCode)
          .timeout(const Duration(seconds: 15), onTimeout: () {
        throw Exception('Request timeout after 15 seconds');
      });

      // Check if cancelled after API call
      if (_isCancelled) {
        logInfo(
            'CurrencyService: Fetch cancelled for $currencyCode after API call');
        return CurrencyFetchResult(
            rate: _staticRates[currencyCode] ?? 1.0,
            status: CurrencyStatus.failed);
      }

      if (result > 0) {
        final fetchDate = DateTime.now();
        // Only log if not cancelled to avoid spam
        if (!_isCancelled) {
          logInfo('CurrencyService: Got live rate for $currencyCode: $result');
        }
        return CurrencyFetchResult(
            rate: result, status: CurrencyStatus.success, fetchDate: fetchDate);
      } else {
        // Only log if not cancelled to avoid spam
        if (!_isCancelled) {
          logInfo(
              'CurrencyService: Invalid result for $currencyCode, using static rate');
        }
        return CurrencyFetchResult(
            rate: _staticRates[currencyCode] ?? 1.0,
            status: CurrencyStatus.failed);
      }
    } catch (e) {
      // Only log if not cancelled to avoid spam
      if (!_isCancelled) {
        logError(
            'CurrencyService: Failed to get live rate for $currencyCode: $e');
      }
      return CurrencyFetchResult(
          rate: _staticRates[currencyCode] ?? 1.0,
          status: CurrencyStatus.failed);
    }
  }

  // Update current rates (used by cache service)
  static void updateCurrentRates(Map<String, double> newRates) {
    _currentRates = Map<String, double>.from(newRates);
    _lastUpdated = DateTime.now();
    _isUsingLiveRates = true;
    logInfo(
        'CurrencyService: Current rates updated with ${newRates.length} currencies');
  }

  // Reset to static rates (used when clearing cache)
  static void resetToStaticRates() {
    _currentRates = Map<String, double>.from(_staticRates);
    _lastUpdated = null;
    _isUsingLiveRates = false;
    _currencyStatus.clear();
    _currencyFetchTimes.clear();
    logInfo('CurrencyService: Reset to static rates');
  }

  // Check if API is available
  static bool get isLiveDataAvailable =>
      true; // Set to true now that we have real API integration

  // Get currency status
  static CurrencyStatus getCurrencyStatus(String currencyCode) {
    return _currencyStatus[currencyCode] ?? CurrencyStatus.staticRate;
  }

  // Get all currency statuses
  static Map<String, CurrencyStatus> get currencyStatuses =>
      Map.unmodifiable(_currencyStatus);

  // Get localized status text
  static String getLocalizedStatus(String currencyCode, dynamic l10n) {
    final status = getCurrencyStatus(currencyCode);
    switch (status) {
      case CurrencyStatus.success:
        return l10n?.currencyStatusSuccess ?? 'Live rate';
      case CurrencyStatus.failed:
        return l10n?.currencyStatusFailed ?? 'Failed to fetch';
      case CurrencyStatus.timeout:
        return l10n?.currencyStatusTimeout ?? 'Timeout';
      case CurrencyStatus.notSupported:
        return l10n?.currencyStatusNotSupported ?? 'Not supported';
      case CurrencyStatus.staticRate:
        return l10n?.currencyStatusStatic ?? 'Static rate';
      case CurrencyStatus.fetchedRecently:
        return l10n?.currencyStatusFetchedRecently ?? 'Recently fetched';
      case CurrencyStatus.fetching:
        return 'Đang fetch...';
    }
  }

  // Get localized status description
  static String getLocalizedStatusDescription(
      String currencyCode, dynamic l10n) {
    final status = getCurrencyStatus(currencyCode);
    switch (status) {
      case CurrencyStatus.success:
        return l10n?.currencyStatusSuccessDesc ??
            'Successfully fetched live rate';
      case CurrencyStatus.failed:
        return l10n?.currencyStatusFailedDesc ??
            'Failed to fetch live rate, using static fallback';
      case CurrencyStatus.timeout:
        return l10n?.currencyStatusTimeoutDesc ??
            'Request timed out, using static fallback';
      case CurrencyStatus.notSupported:
        return l10n?.currencyStatusNotSupportedDesc ??
            'Currency not supported by API';
      case CurrencyStatus.staticRate:
        return l10n?.currencyStatusStaticDesc ?? 'Using static exchange rate';
      case CurrencyStatus.fetchedRecently:
        return l10n?.currencyStatusFetchedRecentlyDesc ??
            'Successfully fetched within the last hour';
      case CurrencyStatus.fetching:
        return 'Đang lấy tỷ giá trực tuyến';
    }
  }

  // Get currency value status (separate from fetch status)
  static CurrencyValueStatus getCurrencyValueStatus(String currencyCode) {
    final fetchStatus = getCurrencyStatus(currencyCode);
    final fetchTime = getCurrencyLastFetchTime(currencyCode);

    // If not successful/fetchedRecently or no fetch time, it's static
    if ((fetchStatus != CurrencyStatus.success &&
            fetchStatus != CurrencyStatus.fetchedRecently) ||
        fetchTime == null) {
      return CurrencyValueStatus.static;
    }

    // Check if recently updated (within 1 hour)
    final now = DateTime.now();
    final difference = now.difference(fetchTime);
    if (difference.inHours < 1) {
      return CurrencyValueStatus.recentlyUpdated;
    }

    // Has fetch data but older than 1 hour
    return CurrencyValueStatus.updated;
  }

  // Get localized value status text
  static String getLocalizedValueStatus(String currencyCode, dynamic l10n) {
    final valueStatus = getCurrencyValueStatus(currencyCode);
    switch (valueStatus) {
      case CurrencyValueStatus.recentlyUpdated:
        return 'Cập nhật gần đây';
      case CurrencyValueStatus.updated:
        return 'Đã cập nhật';
      case CurrencyValueStatus.static:
        return 'Static';
    }
  }

  // Get localized value status description
  static String getLocalizedValueStatusDescription(
      String currencyCode, dynamic l10n) {
    final valueStatus = getCurrencyValueStatus(currencyCode);
    switch (valueStatus) {
      case CurrencyValueStatus.recentlyUpdated:
        return 'Cập nhật trong vòng 1 giờ qua';
      case CurrencyValueStatus.updated:
        return 'Có dữ liệu trực tuyến nhưng cũ hơn 1 giờ';
      case CurrencyValueStatus.static:
        return 'Sử dụng tỷ giá tĩnh';
    }
  }

  // Update status to fetching for currencies being fetched
  static void markCurrencyAsFetching(String currencyCode) {
    _currencyStatus[currencyCode] = CurrencyStatus.fetching;
  }

  // Update status transitions before fetch
  static void updateStatusBeforeFetch() {
    final now = DateTime.now();

    // Update all currency statuses according to new rules
    for (final currencyCode in _currencyStatus.keys.toList()) {
      final currentStatus = _currencyStatus[currencyCode];
      final fetchTime = getCurrencyLastFetchTime(currencyCode);

      if (currentStatus == null) continue;

      switch (currentStatus) {
        case CurrencyStatus.success:
          // Success becomes recently fetched
          _currencyStatus[currencyCode] = CurrencyStatus.fetchedRecently;
          break;
        case CurrencyStatus.fetchedRecently:
          // Check if it's been more than 1 hour
          if (fetchTime != null) {
            final difference = now.difference(fetchTime);
            if (difference.inHours >= 1) {
              _currencyStatus[currencyCode] = CurrencyStatus.staticRate;
            }
          }
          break;
        case CurrencyStatus.failed:
        case CurrencyStatus.timeout:
        case CurrencyStatus.staticRate:
        case CurrencyStatus.notSupported:
          // These will be marked as fetching when actually being fetched
          break;
        case CurrencyStatus.fetching:
          // Already fetching, keep as is
          break;
      }
    }
  }

  // Update status transitions after loading from cache
  static void updateStatusAfterLoad() {
    final now = DateTime.now();

    // Update all currency statuses according to current time
    for (final currencyCode in _currencyStatus.keys.toList()) {
      final currentStatus = _currencyStatus[currencyCode];
      final fetchTime = getCurrencyLastFetchTime(currencyCode);

      if (currentStatus == null) continue;

      switch (currentStatus) {
        case CurrencyStatus.success:
          // Success should become recently fetched if cache was saved after a successful fetch
          _currencyStatus[currencyCode] = CurrencyStatus.fetchedRecently;
          logInfo(
              'CurrencyService: Updated $currencyCode from success to fetchedRecently after cache load');
          break;
        case CurrencyStatus.fetchedRecently:
          // Check if it's been more than 1 hour since fetch
          if (fetchTime != null) {
            final difference = now.difference(fetchTime);
            if (difference.inHours >= 1) {
              _currencyStatus[currencyCode] = CurrencyStatus.staticRate;
              logInfo(
                  'CurrencyService: Updated $currencyCode from fetchedRecently to staticRate (>1hr old)');
            }
          }
          break;
        case CurrencyStatus.failed:
        case CurrencyStatus.timeout:
        case CurrencyStatus.staticRate:
        case CurrencyStatus.notSupported:
        case CurrencyStatus.fetching:
          // Keep these statuses as-is
          break;
      }
    }
  }
}

class Currency {
  final String code;
  final String name;
  final String symbol;

  const Currency(this.code, this.name, this.symbol);

  @override
  String toString() => '$code - $name';

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is Currency &&
          runtimeType == other.runtimeType &&
          code == other.code;

  @override
  int get hashCode => code.hashCode;
}
