...
class P2PDiscoveryService extends ChangeNotifier {
  ...
  Future<void> startDiscovery() async {
    ...
    // 🔥 NEW: Limited discovery for 10 seconds only when enabling network
    logInfo(
        'P2PDiscoveryService: Starting limited 10-second discovery period...');
    ...
    // Schedule auto-stop after 10 seconds to save battery
    Timer(const Duration(seconds: 10), () {
      if (_isDiscovering) {
        logInfo(
            'P2PDiscoveryService: Stopping automatic discovery after 10 seconds (battery optimization)');
        _isDiscovering = false;
        notifyListeners();
      }
    });

    logInfo('P2PDiscoveryService: Discovery started (will auto-stop in 10s)');
    notifyListeners();
  }
