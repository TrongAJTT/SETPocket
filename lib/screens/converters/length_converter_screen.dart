...
class _LengthConverterScreenState extends State<LengthConverterScreen> {
  LengthViewMode _viewMode = LengthViewMode.cards;
  final LengthConverter _converter = LengthConverter();

  // Unit visibility
  final Set<String> _visibleUnits = {'kilometers', 'meters', 'miles'};
  ...
  Future<void> _loadState() async {
    try {
      final state = await LengthStateService.loadState();
      ...
      print(
          'LengthConverter: Loaded state with ${_rowControllers.length} cards');
    } catch (e) {
      print('LengthConverter: Error loading state: $e');
      // Fallback to default
      _addRow();
    }
  }
  ...
  Future<void> _saveState() async {
    try {
      final cards = <LengthCardState>[];

      for (int i = 0; i < _rowControllers.length; i++) {
        final baseUnit = _rowBaseUnits[i];
        final amount = _rowValues[i][baseUnit] ?? 1.0;

        cards.add(LengthCardState(
          unitCode: baseUnit,
          amount: amount,
        ));
      }

      final state = LengthStateModel(
        cards: cards,
        visibleUnits: _visibleUnits.toList(),
        lastUpdated: DateTime.now(),
      );

      await LengthStateService.saveState(state);
      print('LengthConverter: Saved state with ${cards.length} cards');
    } catch (e) {
      print('LengthConverter: Error saving state: $e');
    }
  }
  ...
  void _addRow() {
    ...
    // Use meters as default
    const defaultUnit = 'meters';
  ...
  void _showUnitCustomization() {
    ...
    showDialog(
      ...
      builder: (context) => UnitCustomizationDialog(
        title: AppLocalizations.of(context)!.customizeLengthUnits,
  ...
  void _showInfoDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (context) => _LengthInfoDialog(),
    );
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: l10n.lengthConverterInfo,
          ),
          IconButton(
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: _viewMode == LengthViewMode.cards
                ? _buildCardsView()
                : _buildTableView(),
          ),
        ],
...
class _LengthInfoDialog extends StatelessWidget {
  const _LengthInfoDialog();
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      AppLocalizations.of(context)!.lengthConverterInfo,
                ...
                child: Column(
                  ...
                  children: [
                    _buildSection(
                      ...
                      content:
                          'Convert between 12 different length units, from microscopic Angstroms to vast Nautical Miles. Supports scientific notation for extreme values.',
                    ...
                    _buildSection(
                      ...
                      content:
                          'Angstroms (Å), Nanometers (nm), Microns (μm), Millimeters (mm), Centimeters (cm), Meters (m), Kilometers (km), Inches (in), Feet (ft), Yards (yd), Miles (mi), Nautical Miles (nmi)',
                      icon: Icons.list,
                    ),
                  ],
                ),
