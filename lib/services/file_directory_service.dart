...
class FileDirectoryService {
  ...
  static Future<FilePickerResult?> pickFilesByCategory(
    ...
    try {
      logInfo('$_tag: Opening file picker for category: ${category.label}');
      ...
      final result = await FilePicker.platform.pickFiles(
        ...
        dialogTitle: 'Select ${category.label}',
      );

      if (result != null) {
        logInfo(
            '$_tag: Selected ${result.files.length} files from ${category.label}');
      } else {
        logInfo('$_tag: User cancelled file selection for ${category.label}');
      }

      return result;
    } catch (e) {
      logError('$_tag: Error picking files for category ${category.label}: $e');
      return null;
    }
  }
